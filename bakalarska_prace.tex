...
\begin{flushright}
    ...
    za cenné rady a pomoc při tvorbě bakalářské práce. Zároveň děkuji MUDr. Janu Terčovi a MUDr. Filipovi Roubíčkovi za odborné konzultace a připomínky k medicínsky zaměřeným částem práce. V neposlední řadě děkuji své manželce za trpělivost a psychickou podporu během celého studia i při psaní této práce.
\end{flushright}
...
Práce je členěna do jedenácti kapitol a její hlavní obsah je rozdělen na teoretickou a praktickou část. Teoretická část, zahrnující kapitoly 1 až 5, se věnuje medicínskému a metodickému kontextu řešené problematiky. Nejprve shrnuje základní poznatky o zobrazování krční páteře, významu radiologického hodnocení a sledovaných anatomických a radiologických parametrech. Následně uvádí přehled přístupů hlubokého učení v medicínském zobrazování, zejména v úlohách segmentace, detekce a lokalizace klíčových bodů, a představuje modely, knihovny a frameworky využité v této práci.

Praktická část, tvořená kapitolami 6 až 8, popisuje vlastní návrh a experimentální ověření řešení. Zabývá se extrakcí klíčových bodů ze segmentačních masek, použitým datasetem, metodikou experimentů, trénováním modelů a způsobem vyhodnocení. V závěru této části jsou prezentovány a porovnány dosažené výsledky segmentačního a detekčního přístupu.
...
Označíme-li dva body ležící na zadní hraně obratle C2 jako $\mathbf{p}_{C2}^{TR}$ a $\mathbf{p}_{C2}^{BR}$ a analogicky dva body na zadní hraně obratle C7 jako $\mathbf{p}_{C7}^{TR}$ a $\mathbf{p}_{C7}^{BR}$, lze směrové vektory obou hran zapsat jako:
\begin{equation}
...
V oblasti analýzy medicínských obrazů dnes existuje široká škála modelů pro segmentaci anatomických struktur i pro detekci klíčových bodů. Přehled hlavních směrů, principů a vybraných architektur byl uveden v předchozí kapitole. Tato kapitola se bude soustředit na ty modely, které jsou dále využity v praktické části této práce a které tvoří základ experimentálního porovnání.
...
Významná role MONAI spočívá také v transformacích vstupních dat. Pomocí \texttt{\-Compose} jsou skládány celé transformační pipeline, které zahrnují například \texttt{\-Scale\-Intensityd}, \texttt{SpatialPadd}, \texttt{RandSpatialCropd}, \texttt{RandFlipd}, \texttt{RandRotate90d}, \texttt{Rand\-Zoomd}, \texttt{\-Rand\-Gaussian\-Noised} a \texttt{To\-Tensord}. Tyto transformace jsou využity při přípravě dat pro trénink i validaci a zajišťují sjednocení velikosti vstupů, augmentaci i převod do tenzorové reprezentace. Součástí pipeline je i vlastní transformace \texttt{Histogram\-Equalizationd}, která je implementována jako potomkem \texttt{MapTransform} a zapadá tak přímo do MONAI transformačního frameworku.
...
Extrakce klíčových bodů ze segmentačních masek byla realizována v několika fázích, implementovaných ve skriptu \texttt{extraction.py}\footnote{Zdrojový kód je dostupný na GitHubu: \url{https://github.com/JaroslavRadimsky/bp_ki_ujep}}. Cílem bylo získat 23 bodů popisujících pozice obratlů C2 až C7.
...
\begin{itemize}
  \item Funkce \texttt{approx\_quad} hledá čtyřúhelníkové přiblížení kontury pomocí polygonální aproximace. Pokud polygonální aproximace selže, postupuje se alternativně přes metodu \texttt{cv2.\-minArea\-Rect}, která vrací nejmenší obdelník obsahující celou plochu obratle.
  ...
  \item Následně jsou body seřazeny podle pozice dolních bodů předchozího obratle (funkce \texttt{order\_quad\_from\_previous\_bottom}). Například tedy levý horní roh obratle C4 bude bod z kontury, který je nejbližší levému dolnímu rohu obratle C3.
\end{itemize}
...
Bohužel dataset i přes ověřované anotace obsahuje chybně anotované snímky, kde pozice bodů jsou správné, ale jejich názvy jsou prohozené (objevené při experimentování v rámci této práce). Chybně anotované snímky byly v této práci opraveny (chybné anotace ručně prohozeny) za účelem nezkreslených výsledků.
...
\begin{table}[H]
\centering
\caption{Výsledné hodnoty metriky Dice segmentačního modelu na trénovací a validační množině.}
\label{tab:dice_train_val}
\begin{tabular}{lc}
\hline
Množina & Dice \\
\hline
...
Jak ukazují hodnoty v tabulce~\ref{tab:dice_train_val}, model dosáhl vysoké míry překryvu predikovaných a referenčních masek jak na trénovací, tak na validační množině.
...
Použitá síť obsahuje vstupní konvoluční blok, na který navazují reziduální vrstvy a dvojice za sebou řazených hourglass modulů. V konfiguraci modelu je počet stacked bloků nastaven na $n = 2$ (což odpovídá zjednodušené verzi modelu, zvolené pro rychlejší dosažení výsledků), počet výstupních kanálů odpovídá počtu klíčových bodů, tedy 23, a model pracuje s RGB vstupem o velikosti $256 \times 256$ pixelů. Každý výstupní kanál reprezentuje jednu tepelnou mapu odpovídající jednomu anatomickému bodu.
...
\chapter{Analýza výstupů modelů}
...
Protože však není vhodné společně prezentovat výsledky na trénovacích i testovacích datech, byla tato sjednocená množina dále rozdělena podle rozdělení pro fold 0, na němž byly oba modely trénovány. Trénovací část tak obsahovala 3\,963 snímků a testovací část 991 snímků.
...
\begin{table}[H]
\centering
\caption{Bias segmentačního a detekčního přístupu pro jednotlivé metriky na testovací množině.}
\label{tab:bias_seg_det_test}
\begin{tabular}{lcc}
\hline
Metrika & Segmentace & Detekce \\
\hline
Cobb\_C2\_C7\_deg & -0.1397 & -0.1403 \\
SVA\_C2\_C7\_px & 0.1218 & 0.2444 \\
Seg\_C2\_C3\_deg & -0.2366 & 0.1785 \\
Seg\_C3\_C4\_deg & -0.0485 & -0.5209 \\
Seg\_C4\_C5\_deg & -0.0078 & 0.8185 \\
Seg\_C5\_C6\_deg & 0.1422 & 0.1026 \\
Seg\_C6\_C7\_deg & 0.0111 & -0.7190 \\
Slope\_C2\_deg & 0.4660 & -0.3727 \\
\hline
\end{tabular}
\end{table}

Z tabulky~\ref{tab:bias_seg_det_test} je patrné, že bias obou přístupů zůstává u většiny metrik relativně malý. Ve srovnání se segmentačním přístupem se však u detekčního přístupu výrazněji projevuje kladný i záporný bias, zatímco segmentační přístup vykazuje hodnoty biasu obecně bližší nule s vyjímkou metriky \textit{Slope\_C2\_deg}, kterou segmentační model nadhodnocuje.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{images/results/test/seg/hist/Cobb_C2_C7_deg_error_histogram.png}
    \end{minipage}
    \hfill
    \begin{minipage}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{images/results/test/det/hist/Cobb_C2_C7_deg_error_histogram.png}
    \end{minipage}
    \caption{Histogramy chyb výpočtu Cobbova úhlu C2--C7 na testovací množině. Vlevo je segmentační přístup, vpravo přímá detekce klíčových bodů. Chyba je definována jako rozdíl mezi predikovanou a referenční hodnotou.}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{images/results/test/seg/scatter/Cobb_C2_C7_deg_scatter_gt_vs_pred.png}
    \end{minipage}
    \hfill
    \begin{minipage}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{images/results/test/det/scatter/Cobb_C2_C7_deg_scatter_gt_vs_pred.png}
    \end{minipage}
    \caption{Porovnání referenčních a predikovaných hodnot Cobbova úhlu C2--C7 na testovací množině. Vlevo je segmentační přístup, vpravo přímá detekce klíčových bodů. Přerušovaná přímka znázorňuje ideální shodu predikce s referencí.}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{images/results/test/seg/hist/SVA_C2_C7_px_error_histogram.png}
    \end{minipage}
    \hfill
    \begin{minipage}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{images/results/test/det/hist/SVA_C2_C7_px_error_histogram.png}
    \end{minipage}
    \caption{Histogramy chyb výpočtu sagitální vertikální osy C2--C7 na testovací množině. Vlevo je segmentační přístup, vpravo přímá detekce klíčových bodů. Chyba je definována jako rozdíl mezi predikovanou a referenční hodnotou.}
    \label{fig:hist_seg_test_sva_c2_c7}
    \label{fig:hist_det_test_sva_c2_c7}
\end{figure}

Z histogramu chyb detekčního přístupu (obrázek \ref{fig:hist_det_test_sva_c2_c7} je patrné, že rozdělení není zcela symetrické kolem nulové hodnoty a je mírně posunuto do záporných hodnot. To naznačuje tendenci modelu k lehkému podhodnocování referenčních hodnot, tedy k mírně zápornému biasu. Tento jev je patrný zejména u metrik \textit{Seg\_C6\_C7\_deg}, \textit{Seg\_C3\_C4\_deg}, \textit{Slope\_C2\_deg} a v menší míře také u \textit{Cobb\_C2\_C7\_deg}. U ostatních metrik však zůstává rozdělení chyb soustředěno v blízkosti nuly a celkový bias detekčního přístupu je pouze velmi malý.

\begin{figure}[H]
    \centering
    \begin{minipage}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{images/results/test/seg/scatter/SVA_C2_C7_px_scatter_gt_vs_pred.png}
    \end{minipage}
    \hfill
    \begin{minipage}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{images/results/test/det/scatter/SVA_C2_C7_px_scatter_gt_vs_pred.png}
    \end{minipage}
    \caption{Porovnání referenčních a predikovaných hodnot sagitální vertikální osy C2--C7 na testovací množině. Vlevo je segmentační přístup, vpravo přímá detekce klíčových bodů. Přerušovaná přímka znázorňuje ideální shodu predikce s referencí.}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{images/results/test/seg/hist/Slope_C2_deg_error_histogram.png}
    \end{minipage}
    \hfill
    \begin{minipage}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{images/results/test/det/hist/Slope_C2_deg_error_histogram.png}
    \end{minipage}
    \caption{Histogramy chyb výpočtu sklonu obratle C2 na testovací množině. Vlevo je segmentační přístup, vpravo přímá detekce klíčových bodů. Chyba je definována jako rozdíl mezi predikovanou a referenční hodnotou.}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{images/results/test/seg/scatter/Slope_C2_deg_scatter_gt_vs_pred.png}
    \end{minipage}
    \hfill
    \begin{minipage}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{images/results/test/det/scatter/Slope_C2_deg_scatter_gt_vs_pred.png}
    \end{minipage}
    \caption{Porovnání referenčních a predikovaných hodnot sklonu obratle C2 na testovací množině. Vlevo je segmentační přístup, vpravo přímá detekce klíčových bodů. Přerušovaná přímka znázorňuje ideální shodu predikce s referencí.}
...
Časová náročnost byla hodnocena měřením doby preprocessingu, času vlastní inference modelu, času postprocessingu, času výpočtu klinických metrik a celkového end-to-end času zpracování jednoho snímku.

Měření časové náročnosti bylo realizováno v jazyce Python pomocí standardní knihovny \texttt{time}, konkrétně funkcí \texttt{time.perf\_counter()}, která sloužila k zaznamenání začátku a konce jednotlivých částí pipeline. Protože inference probíhala na GPU, bylo před odečtením uplynulého času vždy provedeno synchronizační volání \texttt{torch.cuda.synchronize()} z knihovny PyTorch, aby naměřený čas odpovídal skutečně dokončeným CUDA operacím a nebyl zkreslen jejich asynchronním vykonáváním. Jednotlivé časy byly ukládány po snímcích do tabulky a následně statisticky zpracovány pomocí knihoven \texttt{pandas} a \texttt{numpy}, zejména s využitím funkcí pro výpočet průměru, mediánu, směrodatné odchylky a 95. percentilu.
...
Paměťová náročnost byla hodnocena na stejném výpočetním uzlu a stejné evaluační množině pomocí samostatného benchmarku, který během zpracování jednotlivých snímků sledoval využití pracovní paměti procesu a u CUDA zařízení také paměť GPU. Pracovní paměť procesu byla získávána pomocí systémových volání přes knihovnu \texttt{ctypes}, zatímco pro GPU byly v PyTorch použity funkce \texttt{torch.cuda.memory\_allocated()} a \texttt{torch.cuda.memory\_reserved()}. Průběžné vzorkování zajišťovala třída \texttt{MemoryMonitor} spuštěná v samostatném vlákně pomocí knihovny \texttt{threading}; výsledky byly následně statisticky zpracovány s využitím knihoven \texttt{pandas} a \texttt{numpy}. Pro porovnání metod byl jako hlavní ukazatel zvolen špičkový nárůst alokované GPU paměti během zpracování jednoho snímku (vzorec \ref{eq:gpu_peak_memory_increase}), který vyjadřuje, o kolik se zvýšila alokovaná paměť GPU oproti stavu před začátkem zpracování daného snímku.
...
Je však třeba zohlednit, že použitý detekční model představoval zjednodušenou variantu architektury Stacked Hourglass se dvěma stacky, takže dosažené výsledky odrážejí také zvolený kompromis mezi přesností a výpočetní náročností.
...
Určitým omezením této práce je také skutečnost, že porovnání se segmentačním přístupem nebylo provedeno vůči plně rozvinuté variantě detekčního modelu, ale vůči zjednodušené konfiguraci architektury Stacked Hourglass se dvěma stacky. Přestože takové nastavení umožnilo dosažení nižší výpočetní náročnosti a jednoduššího nasazení, z hlediska metodického srovnání by bylo vhodné ověřit výsledky i s výkonnější, avšak výpočetně náročnější variantou detekčního modelu. Spravedlivější porovnání obou přístupů by tak mělo zahrnovat nejen přesnost a rychlost inference, ale i srovnatelnou míru optimalizace a kapacity použitých architektur.
